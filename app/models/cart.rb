class Cart < ActiveRecord::Base
	...
	def add_combo_from_mobile(combo_info)
		...
		combo_info["combo_options"].each do |combo_option|
			future_order_items = future_order.order_items.build(item_id: combo_option[:dish_id], item_type: "Dish", category_id: combo_option[:id], category_type: "ComboOption")
		end
		return future_order if future_order.save!
	end
	...
	def check_with_incoming_order_for_mobile(current_order, combo_info)
		...
		combo_info[:combo_options].each do |combo_option|
			current_order.order_items.each do |order_item|
				if order_item[:item_id] == combo_option[:dish_id] and order_item[:category_id] == combo_option[:id]
					counting_length += 1
					break
				end
			end
