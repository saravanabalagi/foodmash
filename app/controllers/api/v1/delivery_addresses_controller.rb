class Api::V1::DeliveryAddressesController < ApiApplicationController
	...
	def index
		...
		if @delivery_addresses
			render status: 200, json: {success: true, data: @delivery_addresses.as_json(:include => :area)}
		else
	...
	def create 
		...
		if @delivery_address.save!
			render status: 201, json: {success: true, data: @delivery_addresses.as_json(:include => :area)}
		else
